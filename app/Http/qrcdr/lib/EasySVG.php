...
class EasySVG
{
    ...
    private function _utf8ToUnicode($str)
    {
        ...
        for ($i = 0; $i < mb_strlen($str); $i++) {
            if (function_exists('mb_ord')) {  // in PHP 7.2.0+ use mb_ord
                $thisValue = mb_ord(mb_substr($str, $i, 1), "UTF-8");
            } else {
                $thisValue = $this->utf8_ord(mb_substr($str, $i, 1));
            }
