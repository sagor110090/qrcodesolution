                ...
                <div class="col-span-12 md:col-span-8">
                    <div
                        ...
                        <div class="p-4 border-b n dark:border-neutral-600">
                            <x-tw.input label="QR Code Name" placeholder="QR Code Name" id="name" required
                                class="col-span-2" name="name" value="{{ $name }}" />
                        </div>
                        <x-qrcode.url :url="$url" />
                        <x-qrcode.email :email="$email" :subject="$subject" :message="$message" />
                        <x-qrcode.vcard :vcard_first_name="$vcard_first_name" :vcard_last_name="$vcard_last_name" :vcard_phone_number="$vcard_phone_number" :vcard_mobile="$vcard_mobile"
                            :vcard_email="$vcard_email" :vcard_website="$vcard_website" :vcard_company="$vcard_company" :vcard_job_title="$vcard_job_title" :vcard_address="$vcard_address"
                            :vcard_fax="$vcard_fax" :vcard_city="$vcard_city" :vcard_post_code="$vcard_post_code" :vcard_country="$vcard_country" />
                        <x-qrcode.sms :sms_phone="$sms_phone" :sms="$sms" />
                        <x-qrcode.phone :phone="$call_phone" />
                        <x-qrcode.event :qrCode="$qrCode" />
                        <x-qrcode.social-media :qrCode="$qrCode" />
                        <x-qrcode.location :latitude="$latitude" :longitude="$longitude" />
                        <x-qrcode.text :text_data="$text_data" />
                        <x-qrcode.wifi :network_name="$network_name" :network_password="$network_password" :network_type="$network_type" :wifi_hidden="$wifi_hidden" />
                        <x-qrcode.bitcoin :bitcoin_address="$bitcoin_address" :bitcoin_amount="$bitcoin_amount" />
                        <x-qrcode.pdf :pdf="$pdf" />
