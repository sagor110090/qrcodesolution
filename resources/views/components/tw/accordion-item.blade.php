...
<div class="border border-neutral-200 bg-white dark:bg-gray-900/50 dark:border dark:border-gray-200/10"
    ...
    <h2 class="mb-0" id="heading{{$id}}">
        <button
            ...
            type="button">
           {{ $label }}
                <div x-text="open ? '-' : '+'" class="ml-auto text-2xl font-bold transition transform group-hover:translate-x-[-2px]">
                </div>
        </button>
    </h2>
    <div id="{{$id}}" class="border-t border-neutral-200" x-show="open" x-on:click="open = false" x-transition>
        <div class="px-5 py-4">
           {{ $slot }}
        </div>
    </div>
