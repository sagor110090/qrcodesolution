...
<x-layouts.frontend>
    ...
    <x-slot name="seo">
        <title>Blog | Qrcode Solution</title>
        ...
        <meta itemprop="name" content="Blog | Qrcode Solution">
        ...
        <meta property="og:title" content="Blog | Qrcode Solution">
        ...
        <meta name="twitter:title" content="Blog | Qrcode Solution">
    ...
    <x-ui.frontend.breadcrumbs :crumbs="[ ['text' => 'Blog'] ]" />
    @volt('blog.index')
        <div class="relative flex flex-col w-full px-6 py-10 mx-auto lg:max-w-6xl sm:max-w-xl md:max-w-full sm:pb-16">
            ...
            <p class="w-full mt-2 text-base font-medium text-neutral-400 dark:text-slate-400 md:mt-2">
                Here our latest posts about qrcode and other stuff.
            </p>
                ...
                @foreach ($posts as $post)
                    <article class="flex space-x-8">
                        <a   href="/{{ $this->route_prefix }}/{{ $post->slug }}" class="block w-5/12">
                            @if ($post->thumbnail)
                                <img src="@if(str_starts_with($post->thumbnail, 'https') || str_starts_with($post->thumbnail, 'http')){{ $post->thumbnail }}@else{{ asset('storage/' . $post->thumbnail) }}@endif" class="relative object-cover w-full h-full bg-gray-200 rounded-lg shadow-lg aspect-video dark:bg-slate-700" alt="{{ $post->title }}" />
                            @else
